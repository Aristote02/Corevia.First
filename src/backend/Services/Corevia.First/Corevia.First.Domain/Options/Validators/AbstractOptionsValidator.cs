...
public abstract class AbstractOptionsValidator<T> : OptionsValidator, IValidateOptions<T> where T : class, ICoreviaFirstOptions
{
    ...
    protected void ValidateProperties(T options, Action<string, string>[] validators, List<string> propertiesToValidate)
    {
        ...
        properties = propertiesToValidate.Count != 0
            ? properties.Where(x => propertiesToValidate.Contains(x.Name) && x.Name != nameof(ICoreviaFirstOptions.SectionName)).ToArray()
            : properties;
